...
function pesquisarFotos(listaFotos, termoBuscado) {

    let listaBuscada = [];

    for (let i = 0; i < listaFotos.length; i++) {
        if (listaFotos[i].tags.includes(termoBuscado)) {
            listaBuscada.push(listaFotos[i]);
        }
    }
    return listaBuscada;
}

console.log(pesquisarFotos(listaFotos, 'cachorro'));
...
/*  Outra forma de fazer :
    ...
    const listaFiltrada = listaFotos.filter( item => 

        item.tags.includes(termoBuscado)
        );
